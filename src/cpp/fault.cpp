                      ...
                      const double &dtime )
{
  ...
  for( int k=0; k<(_nL*_nD); k++ )  //Bruce: cellidx(i,j), return an int i* _nD + j
    {
      /* Compute the amount of slip deficity to add*/
      double slipDeficit = dtime*( plateVelocity - creepVel[k] );
      // std::cerr << "slipDeficit: "<<slipDeficit<<std::endl;

      /* Add the remaining loading */
      _slipDeficit[k] += slipDeficit;
      //if(_slipDeficit[k] < 0){
	//      std::cerr << "Fault loading warning: Negative Slip Deficit!" << std::endl;
  ...
  const
{
  ...
  for (int l=0; l< _nD; ++l)
	...
	stress += _Stiffness(ik,j,l) * ( -_slipDeficit[ cellidx(k,l) ] );
    }
...
//------------------------------------------------------------------------------
/**
  Update static friction on the fault using the log healing function. Drop to dynamic level after an earthquake. 
*/
/*
void Fault::getStaticStrength( const std::vector<double> &EQtime, 
                               const double &tNow)
{
  if( EQtime.size() != _nL*_nD ){
    std::cerr<< "getStaticStrength ERROR: check dimension of EQtime! " << std::endl;
    std::terminate();
  }
  #pragma omp parallel for
    for( int k=0; k<(_nL*_nD); k++){
      if( tNow - EQtime[k] <= _tHeal[k] && EQtime[k] > 0){
        _strengthStatic[k] = _strengthDynamic[k] + _scaling[k] * log( 1.0 + ((tNow-EQtime[k])/_tHeal[k]));
      }
      else{
        _strengthStatic[k] = _initStrengthStatic[k]; 
      }
    }
  return;
}
...
double Fault::getStressAvg( std::vector<double> &stress ) 
  const
{
  if(stress.size() != _nL*_nD){
    stress.resize( _nL*_nD);
  ...
  for (int i=0; i<_nL; ++i)
    for(int j=0; j<_nD; ++j)
    {
      sumStress += stress[cellidx(i,j)];
    }
  double avg = sumStress/(_nL*_nD);
  return avg;     
}

double Fault::getStressDev(std::vector<double> &stress,
  ...
  const
{
  ...
  for(int i=0; i<_nL; ++i)
    for(int j=0; j<_nD; ++j)
    {
      ero += pow((stress[cellidx(i,j)] - average),2);
    }
  double dev = sqrt(ero/(_nL*_nD));
  return dev;
}
...
double Fault::getCellStrainRate( const int &k, const double &stress,
         ...
         const double &activationE )
  const
{
  /* Set up the creep law parameters for this cell */
  CreepLaw C( _arrhA[k], activationE, _stressExpon[k] );
...
double Fault::getCellCreepVelocity( const int &k, const double &stress,
  ...
  const
{
  return _faultWidth * getCellStrainRate( k, stress, temperature, activationE ); 
}
...
void Fault::getCreepVelocity( std::vector<double> &creepVel,
                              ...
                              const std::vector<double> &activationE )
  const
{
  ...
  for( int k=0; k<_nL*_nD;  k++ )
    creepVel[k] = getCellCreepVelocity( k, stress[k], temperature[k], activationE[k] );
  return;
}
  ...
  const
{
  ...
  double minTime = 3e5; // maximum time step = 3 days

  int nCells = _nL*_nD;
  /* container of failure at each cell */
  std::vector<double> failTime(nCells,minTime);
  ...
  #pragma omp parallel
  {
    ...
    for( int i=0; i<_nL; i++ ){
      for( int j=0; j<_nD; j++ ){
        ...
        double stressToFailure = _initStrengthStatic[iCell] - stress[iCell];

        /* Check if already at failure */
        if( stressToFailure <= 0 ){
	        std::cerr<< "Warning: estimateTimeToFailure, cell already critial!"<<std::endl;
          failTime[iCell] = 0.0;
        }
        ...
        /* Convert the loading rate from all cells on the fault into a stress
          ...
          if(stressingRate <= 0.0){
            failTime[iCell] = minTime;
          }
          else{
            failTime[iCell] = stressToFailure / stressingRate;
          }
        } // end of else
  ...
  minTime = *std::min_element(failTime.begin(),failTime.end()); // std::min_element returns iterator to the first minimum
  ...
  const
{
  ...
  for( int i=0; i<_nL; i++ )
    for( int j=0; j<_nD; j++ )
      {
        int k = cellidx(i,j);
        double overstress = stress[k] - _initStrengthStatic[k];
  ...
  const
{
  ...
  for( int i=0; i<_nL; i++ )
    for( int j=0; j<_nD; j++ ){
      ...
      if( isStatic[k] ){
        strength = _initStrengthStatic[k];
      }
      else{
      ...
      if( stress[k] >= strength ){
        criticalIdxs.push_back( k );  //B, add new element at the end of vector 
        nCrit++;
      }
    } // END loop through fault cells
                               ...
                               const double &recordTime)
{
    ...
    for( unsigned int iCrit=0; iCrit < nCritical; iCrit++ )
      {
        ...
        double stressdrop = stress[k] - _strengthArrest[k];
  ...
  if( time >= recordTime){
    for( unsigned int k=0; k<_nL*_nD; k++){
      if ( !isStatic[k] ){
        fault_EQtime[k] = time;
      }
    }
