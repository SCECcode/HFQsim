  ...
  */
  /** Overloaded Constructor for a fault with no temperature decay **/
    Fault(
        ...
        //const std::vector<double> &scaling, // parameter in healing function, Zoeller et al., 2004
        // const std::vector<double> &tHeal, // time required for complete healing of activation energy
        const std::vector<double> &initStaticStrengths, // initial static strength for each slip cell
        const std::vector<double> &arrestStrengths, // arrest strength for each slip cell
        const std::vector<double> &dynamicStrengths, // dynamic strength for each cell
        const std::vector<double> &dosMatrix, // dynamic overshoot coefficient for fault
        ...
        const std::vector<double> &initStress // initial temperature for each slip cell
    	    ...
    	    _nL( nL ), _nD( nD ), _cellLength(dx), _cellHeight(dz), _faultWidth( w ),
    	    _Stiffness(K),
    	    _dosMatrix( dosMatrix ),
    	    _initStress(initStress),
    	    _initStrengthStatic( initStaticStrengths ),
    	    _strengthArrest( arrestStrengths ),
            ...
            //_strengthStatic(nL*nD, 0.0) {}

  //TODO: Check dimensions of the vectors in the constructor
public:

  /* set initial slip deficit from an input file */
  void setSlipDeficit( const std::vector<double> &inputSlipDef)
  {
    _slipDeficit = inputSlipDef;
  }
  ...
  /** Update static strength on the fault */
  //void getStaticStrength( const std::vector<double> &EQtime,
  //                          const double &tNow);

  /** Store value of static strength */
  //void checkStaticStrength( std::vector<double> &staticStrength)
  //const{
  //  staticStrength = _strengthStatic;
  //}

  /*Get the average stress on the fault*/
  double getStressAvg(std::vector<double> &stress) const;

  /*Get the standard deviation of stress on the fault*/
  double getStressDev(std::vector<double> &stress,
  ...
  double getCellStrainRate( const int &k, const double &stress,
                            ...
                            const double &activationE ) const;
  ...
  void computeEarthquake( std::vector<double> &stress, // IN: current stress on fault, will be updated
                          ...
                          std::vector<double> &fault_EQtime, // update latest EQ time at each cell
...
protected:
  ...
  */
  ...
  /*
  double cellDynamicStress( const int &k ) const{
    return _strengthStatic[k] -
      (_initStrengthStatic[k] - _strengthArrest[k])/_dosMatrix[k];
  }
  */
protected:
