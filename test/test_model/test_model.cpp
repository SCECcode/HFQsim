...
double FaultCoolingModel::density = In::density;

/* Diffusivity of granite */
double FaultCoolingModel::diffusivity = In::diffusivity;  

/* Specific heat capacity for granite*/
double FaultCoolingModel::specHeat = In::specHeat;

/* Fault Width */
double FaultCoolingModel::faultWidth = In::SlipZoneWidth;   

/* Distance away from fault plane, where temperature remains the background temperature */
double FaultCoolingModel::cooling_distance = In::Cooling_distance;
...
int main(int argc, char * argv[])
{
  ...
  std::vector<double> vector_fs(nCells,In::fs); // profile with a single uniform value
    ...
    if( nCrit > 0 ){
      ...
      // produce some input slip deficit
        ...
        //faultvec2txtfile( ofilename_slipdef, slipDeficit*(1.0/Units::m), In::nL, In::nD );
	      ...
	      if ( time/Units::year >= 30 && time/Units::year < 100){          
          /* model time */
...
/* add stress drop files circular(22 23) random walk(34), save final dT, actE?  local magnnitude*/
